...
export default function Products() {
  ...
  const handleSaveProduct = async () => {
    ...
    // Kirim SEMUA field — termasuk supplier_reliability_score — sebagai Number
    const payload = {
  ...
  return (
    <div className="page fade-up">
      ...
      <div className="page-header" style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
        <div>
          <h1>Products</h1>
          <p style={{ color:"var(--text2)" }}>Inventory management — {filtered.length} items</p>
        </div>
        <button
      ...
      {showModal && (
        ...
        >
          <div style={{ background:"#fff", borderRadius:16, width:380, padding:"28px 28px 24px", boxShadow:"0 20px 60px rgba(0,0,0,0.18)", display:"flex", flexDirection:"column", gap:16 }}>
            ...
            {/* ── Row 4: Lead Time + Supplier Score ── */}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
